    ...
    return (
        <span style={{ display: 'inline-block', width: size, height: size, verticalAlign: 'middle' }}>
            <svg
                ...
                height={size}
                viewBox="0 0 24 24"
                style={{ animation: 'fc-spin 0.8s linear infinite' }}
            >
                <circle
                    cx="12"
                    cy="12"
...
function resolveImageSrc(u) {
    ...
    if (rel.includes('/')) {
        ...
        if (
            !cleaned.startsWith('product_images/') &&
            !raw.startsWith('/product_images/') &&
            !raw.startsWith('product_images/')
        ) {
            return raw;
        }
...
function buildEmail({ shopName, issues, tips, canStart }) {
    ...
    let body = 'Guten Tag,\n\n';
    body +=
        'wir haben gerade Ihren Feed geprüft und Möglichkeiten gefunden, wie Sie Ihre Produktdaten verbessern können, um mehr Sichtbarkeit auf unserem Marktplatz zu bekommen. Passen Sie dazu folgende Punkte an:\n';

    if (hasTitel) {
        body += '\nTITEL\n';
        body +=
            '- Einige Produkttitel sind doppelt oder zu kurz. Verwenden Sie aussagekräftige Titel mit Marke, Produkttyp und wichtigsten Merkmalen.\n';
    }
    ...
    if (hasDesc) {
        ...
        if (has('zu kurz') || has('platzhalter') || has('ausführlicher')) {
            body +=
                '- Beschreibungen sind zu kurz oder wirken wie Platzhalter. Bitte mind. 80 Zeichen mit Vorteilen, Material und Einsatzbereich.\n';
        }
        if (has('extern')) {
    ...
    if (hasBilder) {
        body += '\nBILDER\n';
        body +=
            '- Bitte mind. 3 Bilder pro Produkt liefern. Erstes Bild als Freisteller (weißer Hintergrund), dazu Milieu- und Detailbilder.\n';
    }
...
function Pill({ tone, children }) {
    const bg = tone === 'ok' ? '#E8F5E9' : tone === 'warn' ? '#FFF8E1' : tone === 'bad' ? '#FFEBEE' : BRAND_COLOR;
    const fg = tone === 'ok' ? '#1B5E20' : tone === 'warn' ? '#7A5B00' : tone === 'bad' ? '#B71C1C' : '#FFFFFF';
    return (
        <span
            style={{
                display: 'inline-flex',
            ...
            >
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, flex: '1 1 auto' }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                        ...
                        {subtitle ? (
                            <div style={{ marginTop: 2, color: '#6B7280', fontSize: 13, lineHeight: '18px' }}>
                                {subtitle}
                            </div>
                        ) : null}
                    </div>
...
function _Table({ columns, rows, highlight }) {
    return (
        <div
            style={{
                overflowX: 'auto',
                ...
                <tbody>
                    {rows.map((r, i) => {
                        ...
                        return (
                            <tr
                                key={i}
                                style={{ background: isHot ? '#FFF7ED' : 'white' }}
                            >
                                {columns.map((c) => (
                                    <td
                                        key={c.key}
...
function ResizableTable({
    ...
    const [widths, setWidths] = useState(() => Object.fromEntries(columns.map((c) => [c.key, computeInitialWidth(c)])));
                                            ...
                                            {longText ? (
                                                ...
                                                <div
                                                    style={{
                                                        ...
                                                        overflow: 'hidden',
                                                        cursor:
                                                            displayValue && displayValue.length > 30
                                                                ? 'pointer'
                                                                : 'default',
                                                    }}
                                                    onClick={() => {
                    ...
                    >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                ...
                                >
                                    <button
                                        onClick={() => setDescriptionHtmlView(false)}
                                        style={{
                                            padding: '3px 10px',
                                            border: 'none',
                                            fontSize: 10,
                                            fontWeight: 600,
                                            cursor: 'pointer',
                                            background: !descriptionHtmlView ? BRAND_COLOR : '#FFF',
                                            color: !descriptionHtmlView ? '#FFF' : '#374151',
                                        }}
                                    >
                                        Text
                                    </button>
                                    <button
                                        onClick={() => setDescriptionHtmlView(true)}
                                        style={{
                                            padding: '3px 10px',
                                            border: 'none',
...
function CollapsibleList({ title, items, tone, hint, onItemClick }) {
    ...
    return (
        ...
        >
            ...
            {items.length > 500 ? (
                <SmallText>Es werden nur die ersten 500 Werte angezeigt, damit die Ansicht schnell bleibt.</SmallText>
            ) : null}
        </details>
    );
}

function _TextInput({ label, value, onChange, placeholder }) {
    return (
...
function RulesPage({ rules, setRules, onSave, saving, saveError, savedAt, adminToken, updateAdminToken }) {
    ...
    const [shippingText, setShippingText] = useState(() => (rules?.allowed_shipping_mode || []).join(', '));
    const [materialText, setMaterialText] = useState(() => (rules?.allowed_material || []).join(', '));
    const [colorText, setColorText] = useState(() => (rules?.allowed_color || []).join(', '));
        ...
        >
            ...
            >
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    />
                    {savedAt ? (
                        <Pill tone="info">Zuletzt gespeichert {savedAt}</Pill>
                    ) : (
                        <Pill tone="info">Noch nicht gespeichert</Pill>
                    )}
                    <button
                        onClick={() => {
                            const next = {
                ...
                {rulesView === 'checker' ? (
                    <div style={{ marginTop: 12, display: 'grid', gap: 12 }}>
                        <div style={{ display: 'grid', gap: 12 }}>
                                    ...
                                    {(draft.allowed_shipping_mode || []).map((val, idx) => (
                                        <button
                                            ...
                                            onClick={() =>
                                                setDraft((r) => ({
                                                    ...r,
                                                    allowed_shipping_mode: (r.allowed_shipping_mode || []).filter(
                                                        (x) => x !== val,
                                                    ),
                                                }))
                                            }
                                    ...
                                    {(draft.allowed_material || []).map((val, idx) => (
                                        <button
                                            ...
                                            onClick={() =>
                                                setDraft((r) => ({
                                                    ...r,
                                                    allowed_material: (r.allowed_material || []).filter(
                                                        (x) => x !== val,
                                                    ),
                                                }))
                                            }
                            ...
                            >
                                <div style={{ fontSize: 13, fontWeight: 800, color: '#111827' }}>
                                    Lieferumfang Allowlist
                                </div>
                                <SmallText>
                                    Einzelne Lieferumfang-Werte, die trotz Format-Abweichung als gültig gelten.
                                </SmallText>
                                <textarea
                                    rows={2}
                            ...
                            >
                                <div style={{ fontSize: 13, fontWeight: 800, color: '#111827' }}>
                                    Mindestlänge für Titel und Beschreibung
                                </div>
                                <div style={{ marginTop: 10, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) =>
                                                setField('description_min_length', Number(e.target.value || 50))
                                            }
                                            style={{
                                                width: 140,
                                                padding: 10,
                            ...
                            >
                                ...
                                <pre style={{ marginTop: 10, overflowX: 'auto', fontSize: 12, lineHeight: '18px' }}>
                    ...
                    <div style={{ marginTop: 12 }}>
                        ...
                        >
                            <StepCard
                                title="QS/APA Attribute"
                                status="ok"
                                subtitle="Herstellerfeed, Titel, Beschreibung, Abmessungen, Lieferumfang, Material, Farbe, Shoptexte"
                            >
                                <SmallText>
                                    Wir bewerten, ob Pflichtattribute für Inhalte vernünftig gefüllt sind:
                                    Herstellerfeed, gut strukturierte Titel und Beschreibungen, nachvollziehbare
                                    Abmessungen, sauberer Lieferumfang im Format &quot;1x Produkt&quot;, sinnvolle
                                    Material- und Farbangaben sowie neutrale shopbezogene Texte ohne zu viel Werbung.
                                </SmallText>
                            </StepCard>
                            <StepCard
                                title="QS/APA Bilder"
                                status="ok"
                                subtitle="1. Bild, Freisteller, Milieu, Anzahl Bilder"
                            >
                                <SmallText>
                                    Wir prüfen, ob das erste Bild zur Offer passt und keine Dubletten hat, ob es
                                    ausreichend Freisteller- und Milieu-Bilder gibt und wie viele Bilder pro Produkt
                                    vorhanden sind. Daraus entstehen Bildpunkte im QS-Tab.
                                </SmallText>
                            </StepCard>
                        </div>
                    </div>
...
function QsPage({ headers, rows }) {
    ...
    const [autoEnabled, _setAutoEnabled] = useState(true);
                ...
                await Promise.all(
                    batch.map(async (sample) => {
                        ...
                        ratios.forEach((ratio, i) => {
                            ...
                            if (i > 0 && ratio < 0.3) hasMilieu = true;
                        });
    ...
    const autoSuggested = useMemo(() => {
        ...
        if (headers.length && rows.length) {
            ...
            const imgCols = norms
                .filter(
                    (h) => h.norm.startsWith('image_url') || h.norm.startsWith('image') || h.norm.startsWith('img_url'),
                )
                .map((h) => h.raw);
            if (imgCols.length) {
                let totalImgs = 0;
        ...
        if (qsImageSamples.length && freistellerChecks && Object.keys(freistellerChecks).length) {
            ...
            if (firstUrls.length >= 5) {
                ...
                const dupCount = Object.values(urlCounts)
                    .filter((c) => c > 1)
    ...
    const imageRaw = scores.bildmatch + scores.freisteller + scores.millieu + scores.anzahlbilder;
    ...
    const _totalPercent = (total180 / 180) * 100;
    ...
    const scoreReasons = useMemo(() => {
        ...
        if (!titleCol) {
            ...
            const uniq = new Set(vals.filter(Boolean));
            const _dupRate = filled ? 1 - uniq.size / filled : 0;
            const avg = avgLen(titleCol);
            if (scores.titel === 20) {
        ...
        if (qsImageSamples.length >= 5) {
            ...
            reasons.bildmatch =
                scores.bildmatch === 20 ? 'Erstbilder in Ordnung.' : 'Zu wenige Produkte für automatische Prüfung.';
        }
    ...
    const attributeItems = useMemo(() => {
        ...
        const criteria = {
            ...
            titel: [
                '20 P: Fill-Rate >= 90%, Durchschn. Laenge >= 40 Zeichen, Duplikat-Rate <= 8%',
                '10 P: Fill-Rate >= 80%, Durchschn. Laenge >= 25 Zeichen',
                '0 P: Schwellenwerte nicht erreicht',
            ],
            beschreibung: [
                '10 P: Fill-Rate >= 85%, Durchschn. Laenge >= 80 Zeichen',
                '5 P: Fill-Rate >= 75%, Durchschn. Laenge >= 40 Zeichen',
                '0 P: Schwellenwerte nicht erreicht',
            ],
            abmessungen: [
                '10 P: Masse-Erkennung (z.B. 90x200 cm) in >= 60% der Zeilen',
                '5 P: Masse-Erkennung in >= 30% der Zeilen',
                '0 P: Masse-Erkennung < 30%',
            ],
            lieferumfang: [
                "20 P: Fill-Rate >= 70% und Format 'Nx Produkt' in >= 70% der Zeilen",
                '10 P: Fill-Rate >= 40% und Format-Rate >= 35%',
                '0 P: Schwellenwerte nicht erreicht',
            ],
            material: ['10 P: Fill-Rate >= 90%', '5 P: Fill-Rate > 0%', '0 P: Spalte leer oder nicht vorhanden'],
            farbe: [
                '10 P: Fill-Rate >= 90%, davon >= 90% gueltige Werte (keine Platzhalter)',
                '5 P: Fill-Rate >= 60%, davon >= 60% gueltig',
                '0 P: Schwellenwerte nicht erreicht',
            ],
            shoptexte: [
                '10 P: Keine shopbezogenen Texte im Feed (Spalte leer/nicht vorhanden)',
                '0 P: Shopbezogene Texte vorhanden (wird als negativ gewertet)',
            ],
        };
    ...
    const _deliveryExamples = useMemo(() => {
        if (!deliveryCol) return [];
        return sampleUniqueValues(rows, deliveryCol, 20);
    }, [rows, deliveryCol]);

    const _materialExamples = useMemo(() => {
        if (!materialCol) return [];
        return sampleUniqueValues(rows, materialCol, 20);
    }, [rows, materialCol]);

    const _colorExamples = useMemo(() => {
        if (!colorCol) return [];
        return sampleUniqueValues(rows, colorCol, 20);
    }, [rows, colorCol]);

    const _shopExamples = useMemo(() => {
        if (!shopCol) return [];
        return sampleUniqueValues(rows, shopCol, 20);
    }, [rows, shopCol]);

    const [_brandExampleLimit, _setBrandExampleLimit] = useState(5);
    const [_titleExampleLimit, _setTitleExampleLimit] = useState(5);
    const [_descExampleLimit, _setDescExampleLimit] = useState(3);
    const [_dimExampleLimit, _setDimExampleLimit] = useState(3);
    const [_deliveryExampleLimit, _setDeliveryExampleLimit] = useState(3);
    const [_materialExampleLimit, _setMaterialExampleLimit] = useState(5);
    const [_colorExampleLimit, _setColorExampleLimit] = useState(5);
    const [_shopExampleLimit, _setShopExampleLimit] = useState(3);
    ...
    return (
        <div style={{ width: '100%', fontFamily: 'ui-sans-serif, system-ui', boxSizing: 'border-box' }}>
                ...
                <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                        ...
                        <button
                            onClick={() => {
                                if (navigator.clipboard)
                                    navigator.clipboard
                                        .writeText(`Attribute Score ${attributeScore} von 90`)
                                        .catch(() => {});
                            }}
                            style={{
                                padding: '3px 7px',
                    ...
                    >
                        ...
                        <button
                            onClick={() => {
                                if (navigator.clipboard)
                                    navigator.clipboard.writeText(`Bild Score ${imageScore} von 90`).catch(() => {});
                            }}
                            style={{
                                padding: '3px 7px',
                                borderRadius: 999,
                    ...
                    >
                        <div>
                            ...
                            <div style={{ fontSize: 14, fontWeight: 800, color: '#111827', marginTop: 2 }}>
                                {apaEligible ? '✅ Geeignet' : '❌ Nicht geeignet'}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            >
                ...
                <SmallText>
                    Bewertung von Herstellerfeed, Titeln, Beschreibungen, Abmessungen, Lieferumfang und Textattributen.
                    Herstellerfeed wird ausschließlich manuell per Ja/Nein bewertet.
                </SmallText>

                <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: '1fr', gap: 8 }}>
                    {attributeItems.map((item) => {
                        const toneColor =
                            item.status === 'ok' ? '#16A34A' : item.status === 'bad' ? '#DC2626' : '#F59E0B';
                        ...
                        return (
                            ...
                            >
                                                    ...
                                                    style={{
                                                        ...
                                                        border:
                                                            item.value === 20
                                                                ? '1px solid #16A34A'
                                                                : '1px solid #D1D5DB',
                                                        background: item.value === 20 ? '#DCFCE7' : '#FFF',
                                                        fontSize: 12,
                                                        fontWeight: 600,
                                                        cursor: 'pointer',
                                                    }}
                                                    ...
                                                    style={{
                                                        ...
                                                        border:
                                                            item.value === 0
                                                                ? '1px solid #DC2626'
                                                                : '1px solid #D1D5DB',
                                                        background: item.value === 0 ? '#FEE2E2' : '#FFF',
                                                        fontSize: 12,
                                                        fontWeight: 600,
                                                        cursor: 'pointer',
                                                    }}
                                ...
                                {/* Description */}
                                {item.description ? (
                                    <div style={{ fontSize: 11, color: '#4B5563', lineHeight: '16px' }}>
                                        {item.description}
                                    </div>
                                ) : null}
            ...
            >
                ...
                <SmallText>
                    Bewertung von erstem Bild, Freistellern, Milieu und Anzahl Bilder. Alle Kriterien werden automatisch
                    erkannt. Werte können manuell angepasst werden.
                </SmallText>
                ...
                {/* Bildanalyse Details */}
                {qsImageSamples.length > 0 &&
                    Object.keys(freistellerChecks || {}).length > 0 &&
                    (() => {
                        const samples = qsImageSamples.slice(0, 20);
                        let checked = 0;
                        let frei = 0;
                        ...
                        const dupFirst = Object.values(urlCounts)
                            .filter((c) => c > 1)
                            .reduce((sum, c) => sum + c, 0);
                        ...
                        return (
                            ...
                            >
                                <div style={{ fontWeight: 700, color: '#111827', fontSize: 12, marginBottom: 4 }}>
                                    Automatische Bildanalyse ({checked} Produkte geprüft)
                                </div>
                                {checked > 0 && (
                                    <div>
                                        Freisteller: {frei}/{checked} ({Math.round((frei / checked) * 100)}%) | Milieu:{' '}
                                        {mil}/{checked} ({Math.round((mil / checked) * 100)}%)
                                    </div>
                                )}
                                <div>
                                    Erstbilder: {uniqueFirst} einzigartig, {dupFirst} doppelt | Durchschn.{' '}
                                    {avgImageCount.toFixed(1)} Bilder/Produkt
                                </div>
                                {freistellerLoading && (
                                    <div
                                        style={{
            ...
            {result ? (
                <div style={{ marginTop: 14, display: 'grid', gap: 12 }}>
                    <StepCard
                        title="Feedback"
                        status={
                            result?.feedback?.issues?.length ? (result?.feedback?.enoughImages ? 'warn' : 'bad') : 'ok'
                        }
                        ...
                        <div style={{ display: 'grid', gap: 8 }}>
                            {typeof result?.feedback?.score === 'number' ? (
                                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                                    <Pill
                                        tone={
                                            result.feedback.score >= 80
                                                ? 'ok'
                                                : result.feedback.score >= 50
                                                  ? 'warn'
                                                  : 'bad'
                                        }
                                    >
                            ...
                            {typeof result?.feedback?.offerCount === 'number' ? (
                                <div style={{ fontSize: 12, color: '#6B7280' }}>
                                    Angebote geprüft: {result.feedback.offerCount}
                                </div>
                            ) : null}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        <div>
                            <div style={{ fontSize: 13, fontWeight: 900, color: '#111827', marginBottom: 8 }}>
                                Original
                            </div>
                            <div
                                style={{
        ...
        return (
            ...
            >
                <circle
                    ...
                    r="8"
                />
                <path d="M10 5v1m0 8v1M7.5 8.5a2.5 2 0 015 0c0 1.5-2.5 2-2.5 3.5m0 0a2.5 2 0 005 0" />
            </svg>
        );
        ...
        return (
            ...
            >
                <circle
                    ...
                    r="8"
                />
                <path d="M7.5 7.5a2.5 2.5 0 015 .8c0 1.7-2.5 2-2.5 3.7" />
                <circle
                    cx="10"
                    cy="15"
                    r=".6"
                ...
                <div style={{ flex: '0 1 50%', minWidth: 0, display: 'grid', gap: 12, alignContent: 'start' }}>
                    ...
                    >
                        <div style={{ fontSize: 13, fontWeight: 600, color: '#111827', marginBottom: 10 }}>
                            Wie möchten Sie Ihren Feed übermitteln?
                        </div>
                        <div
                            style={{
                                display: 'grid',
                        ...
                        >
                            <button
                                ...
                                style={{
                                    ...
                                    fontWeight: 600,
                                    cursor: 'pointer',
                                    background: uploadMethod === 'server' ? MC_BLUE : '#FFF',
                                    color: uploadMethod === 'server' ? '#FFF' : '#374151',
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    fontWeight: 600,
                                    cursor: 'pointer',
                                    background: uploadMethod === 'upload' ? MC_BLUE : '#FFF',
                                    color: uploadMethod === 'upload' ? '#FFF' : '#374151',
                                }}
                            >
                        ...
                        {uploadMethod === 'server' && (
                            <div style={{ marginTop: 12, display: 'grid', gap: 8 }}>
                                {[
                                    {
                                        label: 'FTP-Link',
                                        value: 'ftp://partner31679@partnerftp.shopping.check24.de:44021/inbound/offerfeed_MeinShop.csv',
                                        mono: true,
                                    },
                                    ...
                                    >
                                        ...
                                        >
                                            {r.value}{' '}
                                            {r.copy && <span style={{ cursor: 'pointer', color: '#9CA3AF' }}>⧉</span>}
                                        </div>
                                    </div>
                                ))}
                            </div>
...
function _McDashboard() {
    const kpis = [
        ...
        },
        {
            ...
            highlight: true,
            tooltip:
                'Der Content Score zeigt den Anteil fehlerfreier Zeilen in Ihrem Feed. Ab 70/100 kann der Feed freigeschaltet werden. Klicken Sie hier, um zur Analyse zu gelangen.',
        },
    ];
                    ...
                    {kpis.map((k) => (
                        ...
                        >
                            ...
                            <div style={{ display: 'flex', alignItems: 'baseline', gap: 3, marginTop: 4 }}>
                                <span style={{ fontSize: 26, fontWeight: 700, color: k.color, lineHeight: 1 }}>
                                    {k.value}
                                </span>
                                <span style={{ fontSize: 13, color: k.color, fontWeight: 600 }}>{k.unit}</span>
                                <span
                                    style={{ fontSize: 10, color: '#9CA3AF', marginLeft: 'auto', whiteSpace: 'nowrap' }}
                                >
                                    <span
                                        style={{
                                            fontWeight: 600,
                            ...
                            {k.highlight && (
                                <div style={{ marginTop: 6 }}>
                                    <span style={{ fontSize: 11, color: MC_BLUE, textDecoration: 'underline' }}>
                                        Zur Analyse
                                    </span>
                                </div>
                            )}
            ...
            <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    {[
                        {
                            value: '188.136,56 €',
                            label: 'Umsatz aus versendeten Bestellungen\nin diesem Kalendermonat',
                        },
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 32, flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 13, color: '#374151' }}>
                        <svg
                            ...
                            strokeWidth="2"
                        >
                            <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.8 19.79 19.79 0 01.22 1.18 2 2 0 012.22 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 7.91a16 16 0 006.18 6.18l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" />
                        </svg>
                        089 - 2424 1158 300
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 13, color: '#374151' }}>
...
function _McIssueCard({ title, severity, description, items, more, fixInstruction, compactList }) {
...
export default function App() {
    ...
    const [route, _setRoute] = useState(() => 'checker-mc');
    const _supabase = useMemo(() => getSupabaseClient(), []);

    const [rules, setRules] = useState(DEFAULT_RULES);
    const [_rulesLoading, setRulesLoading] = useState(true);
    const [_rulesError, setRulesError] = useState('');
    ...
    const [shopName, _setShopName] = useState('');
    ...
    const [_editingEmail, setEditingEmail] = useState(false);
    ...
    const [optionalFields] = useState(['washable_cover', 'mounting_side']);
    ...
    const autoMapping = useMemo(() => {
        ...
        const candidates = {
            ...
            eprel_registration_number: ['EPREL_registration_number', 'eprel_registration_number', 'eprel', 'eprel_nr'],
        };
    ...
    const _duplicateTitles = useMemo(() => {
    ...
    const optionalFindings = useMemo(() => {
        ...
        for (const f of allMissingFields) {
            if (mapping[f]) missingFieldCols[f] = mapping[f];
        }
        ...
        if (mapping.delivery_includes) {
            try {
                deliveryRe = new RegExp(
                    String(rules?.delivery_includes_pattern ?? DEFAULT_RULES.delivery_includes_pattern),
                    'i',
                );
            } catch (_e) {
                deliveryRe = null;
            ...
            deliveryAllowList = (
                rules?.delivery_includes_allowlist ||
                DEFAULT_RULES.delivery_includes_allowlist ||
                []
            ).map((x) => String(x).trim());
        }
        const dtReUnit =
            /^\s*\d+(?:\s*-\s*\d+)?\s*(tage?|werktage?|arbeitstage?|wochen?|woche|wk\.?|wt\.?|d|days?)\s*$/i;
        const dtReNum = /^\s*\d+(?:\s*-\s*\d+)?\s*$/;
        const shippingAllowed = mapping.shipping_mode
            ? (rules?.allowed_shipping_mode || DEFAULT_RULES.allowed_shipping_mode).map((x) => String(x).toLowerCase())
            : [];
        const shippingModeAliases = {
            package: 'paket',
            pakete: 'paket',
            parcel: 'paket',
            pkg: 'paket',
            karton: 'paket',
            shipment: 'spedition',
            spedition_ware: 'spedition',
            speditionsware: 'spedition',
            freight: 'spedition',
        ...
        if (mapping.mounting_side && !montageInstrRe.test(String(mapping.mounting_side))) {
            ...
            for (let i = 0; i < Math.min(rows.length, 50); i++) {
                ...
                if (v && mountingRe.test(v)) {
                    mountingHits++;
                    if (mountingHits >= 2) break;
                }
            }
            validateMountingSide = mountingHits >= 2;
        }
        const matAllowedBase = mapping.material
            ? (rules?.allowed_material || DEFAULT_RULES.allowed_material).map((x) => String(x).toLowerCase().trim())
            : [];
        const matBlacklist = ['keine angabe'];
        const matAllowed = matAllowedBase.filter((t) => t && !matBlacklist.includes(t));
        const colorAllowed = mapping.color
            ? (rules?.allowed_color || DEFAULT_RULES.allowed_color).map((x) => String(x).toLowerCase().trim())
            : [];
        ...
        for (const field of Object.keys(EXAMPLE_TEMPLATE_VALUES)) {
            const ex = (EXAMPLE_TEMPLATE_VALUES[field] || [])
        ...
        for (let idx = 0; idx < rows.length; idx++) {
            ...
            // Missing fields
            for (const f in missingFieldCols) {
                if (isBlank(r[missingFieldCols[f]])) {
                    if (!missingEansByField[f]) missingEansByField[f] = [];
                    missingEansByField[f].push(ean);
                }
            }
            ...
            if (mapping.washable_cover) {
                const v = String(r[mapping.washable_cover] ?? '')
                    .trim()
                    .toLowerCase();
                if (v && v !== 'ja' && v !== 'nein') invalidWashableCover.push({ ean, value: v });
            }
            // Mounting side – only when the mapped column truly looks like mounting-side data
            if (validateMountingSide) {
                const v = String(r[mapping.mounting_side] ?? '')
                    .trim()
                    .toLowerCase();
                if (v && v !== 'links' && v !== 'rechts' && v !== 'beidseitig')
                    invalidMountingSide.push({ ean, value: v });
            }
            // Delivery time
            if (mapping.delivery_time) {
                const v = String(r[mapping.delivery_time] ?? '').trim();
                if (!v || (!dtReUnit.test(v) && !dtReNum.test(v))) invalidDeliveryTime.push({ ean, value: v });
            }
            // Stock amount – must be a non-negative integer
            if (mapping.stock_amount) {
                const v = String(r[mapping.stock_amount] ?? '').trim();
                if (v && !/^\d+$/.test(v)) invalidStock.push({ ean, value: v });
            }
            // Images
            ...
            if (mapping.shipping_mode) {
                const v = String(r[mapping.shipping_mode] ?? '').trim();
                if (!v) missingShipping.push(ean);
                else {
                    const vl = v.toLowerCase();
                    const canon = shippingModeAliases[vl] || vl;
                    if (!shippingAllowed.includes(canon)) invalidShipping.push({ ean, value: v });
                }
            }
            // Material
            if (matAllowed.length && mapping.material) {
                const v = String(r[mapping.material] ?? '').trim();
                if (v) {
                    const vl = v.toLowerCase();
                    if (!matAllowed.some((t) => vl.includes(t)) || matBlacklist.some((b) => vl.includes(b)))
                        invalidMaterial.push({ ean, value: v });
                }
            }
            // Color
            if (colorAllowed.length && mapping.color) {
                const v = String(r[mapping.color] ?? '').trim();
                if (v && !colorAllowed.some((t) => v.toLowerCase().includes(t))) invalidColor.push({ ean, value: v });
            }
            // Title
            if (mapping.name) {
            ...
            if (mapping.description) {
                ...
                if (desc && titleVal && desc.toLowerCase() === titleVal.toLowerCase())
                    descriptionIssues.templateLike.push(ean);
                else {
                    const wc = desc ? desc.split(/\s+/).filter(Boolean).length : 0;
                    if (wc > 0 && wc <= 3) descriptionIssues.templateLike.push(ean);
                    else if (/beispieltext|musterbeschreibung|lorem ipsum/i.test(desc.toLowerCase()))
                        descriptionIssues.templateLike.push(ean);
                }
            }
            // Template values
            for (const field in templateColumnsMap) {
                if (Object.hasOwn(templateColumnsMap, field)) {
                    const v = String(r[templateColumnsMap[field].col] ?? '').trim();
                    if (v && templateColumnsMap[field].examples.has(v.toLowerCase()))
                        templateValueHits.push({ ean, column: field, value: v });
                }
            }
            // Lighting energy
            if (hasEnergyCols && mapping.name) {
                const t = String(r[mapping.name] ?? '').toLowerCase();
                if (t && lampTokens.some((tok) => t.includes(tok))) {
                    if (
                        (energyCol && isBlank(r[energyCol])) ||
                        (lightingInclCol && isBlank(r[lightingInclCol])) ||
                        (eprelCol && isBlank(r[eprelCol]))
                    )
                        lightingEnergyMissing.push(ean);
                }
            }
        }

        // Deduplicate missing fields
        for (const f in missingEansByField) {
            if (Object.hasOwn(missingEansByField, f))
    ...
    const imageSamples = useMemo(() => {
        ...
        for (let i = 0; i < rows.length; i += 1) {
            ...
            if (!urls.length) continue;
            const id = eanColumn ? String(r?.[eanColumn] ?? '').trim() || `ROW_${i + 1}` : `ROW_${i + 1}`;
    ...
    const summary = useMemo(() => {
        ...
        const _findTargetByRowIndex = (rowIndex) => {
        ...
        if (!mapping.category_path) {
            addIssue(
                'Kategoriepfad fehlt. Ohne Kategoriepfad können nicht zum Sortiment passende Artikel nicht herausgefiltert werden.',
            );
            addTip(
                'Bitte liefern Sie eine Spalte mit dem Kategoriepfad (z. B. "Kategorie", "category" oder "category_path"), idealerweise mit Ebenen, die mit " > " getrennt sind.',
            );
        }
        ...
        (() => {
            ...
            for (let i = 0; i < rows.length; i++) {
                for (const c of textCols) {
                    if (mojibakeRe.test(String(rows[i]?.[c] ?? ''))) {
                        badRows.add(i);
                        break;
                    }
                }
            ...
            if (badRows.size > 0) {
                ...
                addIssue(`Zeichencodierung fehlerhaft in ${badRows.size} Artikeln (z. B. "Ã¤" statt "ä").`, {
                    rowIndices: Array.from(badRows).sort((a, b) => a - b),
                    rowIndex: Math.min(...badRows),
                });
                addTip(
                    'Die Datei bitte als UTF-8 speichern und erneut hochladen. Aktuell sieht es so aus, als wäre Windows-1252-Text fälschlich als UTF-8 gelesen (oder umgekehrt) worden.',
                );
            }
        })();
        ...
        (() => {
            ...
            const germanScore =
                countMatches(
                    /\b(der|die|das|den|dem|des|und|oder|mit|für|von|vom|zur|zum|ein|eine|einen|einer|eines|auf|aus|bei|nicht|auch|sowie|inkl|ohne|als|wie|sehr|bis)\b/g,
                ) + countMatches(/[äöüß]/g);
            // English function words
            const englishScore = countMatches(
                /\b(the|and|with|for|of|from|this|that|these|those|are|is|was|were|has|have|including|without|made|use|used|product|features|material|color|size|height|width|length|depth|black|white|brown|grey|gray)\b/g,
            );
            const total = germanScore + englishScore;
            if (total < 20) return; // not enough signal
            if (englishScore >= germanScore * 2 && germanScore < 10) {
                addIssue(
                    'Der Feed-Inhalt (z. B. Produktname, Beschreibung, Farbe, Lieferumfang) scheint auf Englisch zu sein. Bitte liefern Sie den Content auf Deutsch.',
                );
                addTip(
                    'Übersetzen Sie Produkttitel, Beschreibungen sowie Farb- und Materialangaben ins Deutsche, bevor Sie den Feed erneut hochladen.',
                );
            }
        })();
        ...
        if (eanColumn) {
            ...
            rows.forEach((r, idx) => {
                if (isBlank(r[eanColumn])) {
                    criticalRowIdx.add(idx);
                    missingEanIndices.push(idx);
                }
            });
            if (missingEanIndices.length > 0) {
                addIssue(`EAN fehlt in ${missingEanIndices.length} Artikeln.`, {
                    rowIndices: missingEanIndices,
                    rowIndex: missingEanIndices[0],
                    ean: null,
                });
            }
        } else {
            ...
            addTip(
                'Bitte liefern Sie eine EAN- oder GTIN-Spalte. Falls die Werte in Excel im E-Format stehen, bitte als Text formatieren.',
            );
        }
        ...
        if (eanColumn) {
            if (duplicates.eanDup.size > 0) {
                ...
                addIssue(`Doppelte EAN erkannt in ${duplicates.eanDup.size} Zeilen.`, {
                    rowIndices: dupIndices,
                    rowIndex: dupIndices[0],
                    eans: [...new Set(dupEans)],
                    ean: dupEans[0] || null,
                });
            }
        ...
        if (titleColumn && duplicates.titleDup.size > 0) {
            ...
            const titleDupEans = titleDupIndices
                .map((idx) => (eanColumn ? String(rows[idx]?.[eanColumn] ?? '').trim() : ''))
                .filter(Boolean);
            const titleDupTarget = {
                rowIndices: titleDupIndices,
                rowIndex: titleDupIndices[0],
                eans: [...new Set(titleDupEans)],
                ean: titleDupEans[0] || null,
                column: titleColumn,
            };
            if (mapping.category_path) {
                ...
                addTip(
                    `Doppelte Produkttitel in ${duplicates.titleDup.size} Zeilen. Bei vorhandenem Kategoriepfad werden unpassende Produkte ohnehin herausgefiltert.`,
                    titleDupTarget,
                );
            } else {
        ...
        if (sellerColumn && duplicates.sellerDup.size > 0) {
            ...
            const sellerDupEans = sellerDupIndices
                .map((idx) => (eanColumn ? String(rows[idx]?.[eanColumn] ?? '').trim() : ''))
                .filter(Boolean);
            addIssue(`Doppelte Seller_Offer_ID erkannt in ${duplicates.sellerDup.size} Zeilen.`, {
                rowIndices: sellerDupIndices,
                rowIndex: sellerDupIndices[0],
                eans: [...new Set(sellerDupEans)],
                ean: sellerDupEans[0] || null,
            });
        }
        ...
        if (optionalMissingCount > 0) {
            ...
            addTip('Material, Farbe oder Lieferumfang fehlt.', {
                ...findTargetsByEans([
                    ...optionalFindings.missingEansByField.material,
                    ...optionalFindings.missingEansByField.color,
                    ...optionalFindings.missingEansByField.delivery_includes,
        ...
        if (missingHsCodeCount > 0) {
            ...
            addTip('HS-Code fehlt.', {
                ...findTargetsByEans(optionalFindings.missingEansByField.hs_code),
                column: mapping.hs_code,
            });
        }
        if (missingManufacturerNameCount > 0 || missingManufacturerCountryCount > 0) {
            ...
            addTip('Herstellerangaben fehlen.', {
                ...findTargetsByEans([
                    ...optionalFindings.missingEansByField.manufacturer_name,
                    ...optionalFindings.missingEansByField.manufacturer_country,
                ]),
                column: mapping.manufacturer_name || mapping.manufacturer_country,
            });
        }
        ...
        if (imageColumns.length === 0) {
            ...
            if (lowImageEans.length > 0) {
                ...
                addTip(`Zu wenige Bilder (mind. ${imageMin} empfohlen).`, {
                    ...findTargetsByEans(lowImageEans),
                    column: imageColumns[0],
                });
            }
            if (brokenImageIds.length > 0) {
                addRowsByEans(brokenImageIds, criticalRowIdx);
                addIssue(
                    `Bei ${brokenImageIds.length} Produkten konnten Vorschaubilder nicht geladen werden. Bitte Bild-Links prüfen.`,
                );
            }
        }
        ...
        score -= Math.min(5, missingManufacturerNameCount + missingManufacturerCountryCount > 0 ? 5 : 0);
        ...
        if (mapping.delivery_includes && optionalFindings.invalidDeliveryIncludes.length > 0) {
            ...
            addTip('Lieferumfang-Format ungültig.', {
                ...findTargetsByEans(optionalFindings.invalidDeliveryIncludes.map((x) => x?.ean)),
                column: mapping.delivery_includes,
            });
            score -= 5;
        }
        ...
        if (mapping.delivery_time && optionalFindings.invalidDeliveryTime.length > 0) {
            ...
            addTip('Lieferzeit-Format ungültig.', {
                ...findTargetsByEans(optionalFindings.invalidDeliveryTime.map((x) => x?.ean)),
                column: mapping.delivery_time,
            });
            score -= 5;
        }
        ...
        if (mapping.stock_amount && optionalFindings.invalidStock.length > 0) {
            ...
            addTip('Bestand muss eine Zahl sein.', {
                ...findTargetsByEans(optionalFindings.invalidStock.map((x) => x?.ean)),
                column: mapping.stock_amount,
            });
            score -= 5;
        }
        ...
        if (mapping.description) {
            if (optionalFindings.descriptionIssues.tooShort.length > 0) {
                ...
                const descIssueEans = [
                    ...new Set([
                        ...optionalFindings.descriptionIssues.tooShort,
                        ...(optionalFindings.descriptionIssues.templateLike || []),
                    ]),
                ];
                addTip('Beschreibungen zu kurz oder unvollständig.', {
                    ...findTargetsByEans(descIssueEans),
        ...
        if (mapping.description && optionalFindings.descriptionIssues.externalLinks.length > 0) {
            ...
            addTip('Externe Links in Beschreibungen.', {
                ...findTargetsByEans(optionalFindings.descriptionIssues.externalLinks),
                column: mapping.description,
            });
            score -= 3;
        ...
        if (mapping.category_path && rows.length) {
            ...
            const categoryRecs = [
                ...
                {
                    match: /\bbett\b|betten|boxspring/i,
                    label: 'Betten',
                    attrs: ['Liegefläche', 'Lattenrost inklusive', 'Matratze inklusive'],
                },
                {
                    match: /sofa|couch|ecksofa|wohnlandschaft/i,
                    label: 'Sofas',
                    attrs: ['Sitzhöhe', 'Sitztiefe', 'Bezug abnehmbar'],
                },
                {
                    match: /lampe|leuchte|licht|beleuchtung/i,
                    label: 'Leuchten',
                    attrs: ['Lichtfarbe', 'Leuchtmittel inklusive', 'Energieeffizienzklasse'],
                },
                { match: /stuhl|stühle|sessel|hocker/i, label: 'Stühle', attrs: ['Sitzhöhe', 'Belastbarkeit', 'Stil'] },
                {
                    match: /schrank|kleiderschrank|regal/i,
                    label: 'Schränke/Regale',
                    attrs: ['Anzahl Türen', 'Anzahl Fächer', 'Belastbarkeit pro Fach'],
                },
                {
                    match: /tisch|esstisch|couchtisch|schreibtisch/i,
                    label: 'Tische',
                    attrs: ['Belastbarkeit', 'Ausziehbar', 'Tischhöhe'],
                },
            ];
        ...
        const canStart = score >= 50 && requiredPresence.missing.length === 0 && !!eanColumn;
    ...
    const criticalRowIndexSet = useMemo(() => new Set(summary?.criticalRowIndices ?? []), [summary]);
    const _warningRowIndexSet = useMemo(() => new Set(summary?.warningRowIndices ?? []), [summary]);
    ...
    const rowCriticalIssuesByIndex = useMemo(() => {
        ...
        const tooShortSet = new Set(
            (optionalFindings.descriptionIssues?.tooShort || []).map((x) => String(x ?? '').trim()).filter(Boolean),
        );
        const externalLinksSet = new Set(
            (optionalFindings.descriptionIssues?.externalLinks || [])
                .map((x) => String(x ?? '').trim())
                .filter(Boolean),
        );
    ...
    const toggleIssueRowsOnly = () => {
        ...
        if (next) {
            ...
            if (maxIdx !== null) {
                /* previewCount is managed by the parent scope */
            }
    ...
    const filteredPreviewRows = useMemo(() => {
        ...
        return rows.filter((r) => {
            ...
            return Object.values(r).some((v) => {
                ...
                return termsLower.some((t) => c.includes(t));
            });
        });
    ...
    const jumpToIssueTarget = (target) => {
        if (!target) return;
        setShowIssueRowsOnly(false);
        const targetEans = Array.isArray(target.eans) ? target.eans : target.ean ? [target.ean] : [];
        ...
        if (isXlsx) {
            ...
            reader.onload = (evt) => {
                try {
                    ...
                    if (!firstSheetName) {
                        setParseError('Excel-Datei enthält keine Tabellenblätter.');
                        setParsing(false);
                        return;
                    }
        ...
        const tryParse = (encoding) => {
            ...
            reader.onload = (evt) => {
                ...
                Papa.parse(text, {
                    ...
                    error: (err) => {
                        setParseError(String(err || 'CSV parsing error'));
                        setParsing(false);
                    },
                });
            };
                                ...
                                {pageMode === 'feed-checker' && headers.length ? (
                                    <>
                                            ...
                                            >
                                                ...
                                                <div>
                                                    ...
                                                    <div style={{ fontSize: 12, color: '#7F1D1D', lineHeight: '1.6' }}>
                                                        Es konnten keine bekannten Spalten erkannt werden. Bitte prüfen
                                                        Sie, ob Sie die richtige Datei hochgeladen haben. Erwartete
                                                        Spalten sind z.&nbsp;B. <code>ean</code>, <code>name</code>,{' '}
                                                        <code>price</code>, <code>shipping_mode</code> o.&nbsp;ä.
                                                    </div>
                                                </div>
                                            </div>
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                                ...
                                                                {summary.issues.length ? (
                                                                    ...
                                                                    >
                                                                        {summary.issues.length} kritische Punkte
                                                                        gefunden.
                                                                    </div>
                                                                ) : (
                                                                    <div
                                                            ...
                                                            <div>
                                                                ...
                                                                >
                                                                    Betroffene Zeilen: {summary.criticalRowsCount ?? 0}{' '}
                                                                    von {rows.length} ({summary.criticalRowsPct ?? 0}%)
                                                                </div>
                                                                <ul
                                                                    style={{
                                                                        marginTop: 2,
                                                            ...
                                                            <div>
                                                                ...
                                                                >
                                                                    Betroffene Zeilen: {summary.warningRowsCount ?? 0}{' '}
                                                                    von {rows.length} ({summary.warningRowsPct ?? 0}%)
                                                                </div>
                                                                <ul
                                                                    style={{
                                                                        marginTop: 2,
                                                                    ...
                                                                    <div style={{ display: 'flex', gap: 4 }}>
                                                                        <button
                                                                            onClick={() => {
                                                                                ...
                                                                                setEmailContent(generatedEmail.body);
                                                                            }}
                                                                            style={{
                                                                                padding: '4px 10px',
                                                                                borderRadius: 6,
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                                <div>
                                                                    ...
                                                                    >
                                                                        Alle Zeilen mit Fehlern und Warnungen inkl. EAN,
                                                                        Offer ID, Name und Grund.
                                                                    </div>
                                                                </div>
                                                                <button
                                                                    ...
                                                                    onClick={async () => {
                                                                        ...
                                                                        const missingMaterialSet = new Set(
                                                                            optionalFindings.missingEansByField
                                                                                ?.material || [],
                                                                        );
                                                                        const missingColorSet = new Set(
                                                                            optionalFindings.missingEansByField
                                                                                ?.color || [],
                                                                        );
                                                                        const missingDeliverySet = new Set(
                                                                            optionalFindings.missingEansByField
                                                                                ?.delivery_includes || [],
                                                                        );
                                                                        const imageOneSet = new Set(
                                                                            optionalFindings.imageOneEans || [],
                                                                        );
                                                                        const imageLowSet = new Set(
                                                                            optionalFindings.imageLowEans || [],
                                                                        );
                                                                        const templateSet = new Set(
                                                                            (
                                                                                optionalFindings.descriptionIssues
                                                                                    ?.templateLike || []
                                                                            ).map((x) => String(x ?? '').trim()),
                                                                        );
                                                                        const invalidMatSet = new Set(
                                                                            (
                                                                                optionalFindings.invalidMaterial || []
                                                                            ).map((x) => String(x?.ean ?? '').trim()),
                                                                        );
                                                                        const invalidColSet = new Set(
                                                                            (optionalFindings.invalidColor || []).map(
                                                                                (x) => String(x?.ean ?? '').trim(),
                                                                            ),
                                                                        );
                                                                        ...
                                                                        for (
                                                                            ...
                                                                            for (let idx = start; idx < end; idx++) {
                                                                                const r = rows[idx];
                                                                                const ean = eanColumn
                                                                                    ? String(r[eanColumn] ?? '').trim()
                                                                                    : '';
                                                                                const sellerId = sellerCol
                                                                                    ? String(r[sellerCol] ?? '').trim()
                                                                                    : '';
                                                                                const name = nameCol
                                                                                    ? String(r[nameCol] ?? '').trim()
                                                                                    : '';
                                                                                const reasons = [];
                                                                                if (rowCriticalIssuesByIndex[idx])
                                                                                    reasons.push(
                                                                                        ...rowCriticalIssuesByIndex[
                                                                                            idx
                                                                                        ],
                                                                                ...
                                                                                if (ean) {
                                                                                    if (missingMaterialSet.has(ean))
                                                                                        reasons.push('Material fehlt');
                                                                                    if (missingColorSet.has(ean))
                                                                                        reasons.push('Farbe fehlt');
                                                                                    if (missingDeliverySet.has(ean))
                                                                                        reasons.push(
                                                                                            'Lieferumfang fehlt',
                                                                                        );
                                                                                    if (
                                                                                        ...
                                                                                        reasons.push('Zu wenig Bilder');
                                                                                    if (templateSet.has(ean))
                                                                                        reasons.push(
                                                                                            'Beschreibung wirkt wie Platzhalter',
                                                                                        );
                                                                                    if (invalidMatSet.has(ean))
                                                                                        reasons.push(
                                                                                            'Material ungültig',
                                                                                        );
                                                                                    if (invalidColSet.has(ean))
                                                                                        reasons.push('Farbe ungültig');
                                                                                }
                                                                                if (duplicates.sellerDup?.has(idx))
                                                                                    reasons.push('Doppelte Offer ID');
                                                            ...
                                                            {csvExporting && (
                                                                <div style={{ marginTop: 10 }}>
                                                                    ...
                                                                    >
                                                                        {csvProgress}% analysiert ({rows.length} Zeilen)
                                                                    </div>
                                                                </div>
                                                            )}
                                                        </div>
                                        ...
                                        {(showAllChecks || stage1Status !== 'ok') && (
                                            <StepCard
                                                title="Spalten und Pflichtfelder"
                                                status={stage1Status}
                                                subtitle="Wir prüfen, ob Pflichtinformationen vorhanden sind oder zugeordnet werden können"
                                            >
                                                {!headers.length ? (
                                                    <SmallText>
                                                        Bitte CSV hochladen um die erkannten Spalten zu sehen.
                                                    </SmallText>
                                                ) : (
                                                    <>
                                                        {/* Step 2 status bar */}
                                                            ...
                                                            >
                                                                ...
                                                                <button
                                                                    ...
                                                                    style={{
                                                                        padding: '4px 10px',
                                                                        borderRadius: 999,
                                                                        border: `1px solid ${allRequiredOk ? 'rgba(22,101,52,0.25)' : 'rgba(146,64,14,0.25)'}`,
                                                                        background: '#FFFFFF',
                                                                        fontSize: 11,
                                                                        fontWeight: 600,
                                                                        cursor: 'pointer',
                                                        ...
                                                        {mappingPreviewOpen && (
                                                            ...
                                                            >
                                                                ...
                                                                >
                                                                    Farben: <span style={{ color: '#166534' }}>■</span>{' '}
                                                                    Auto &nbsp;
                                                                    <span style={{ color: '#1D4ED8' }}>■</span> Inhalt
                                                                    erkannt &nbsp;
                                                        ...
                                                        {(!allRequiredOk || step2Expanded) && (
                                                            ...
                                                            >
                                                                <div>
                                                                    <SmallText>
                                                                        Gefundene Spalten {headers.length}. Pflicht sind
                                                                        nur <code>ean (GTIN14)</code>,{' '}
                                                                        <code>seller_offer_id</code> und{' '}
                                                                        <code>name</code>. Alle anderen Felder sind
                                                                        optional.
                                                                    </SmallText>
                                                                    <div
                                                                        style={{
                                                                ...
                                                                >
                                                                    ...
                                                                    <SmallText>
                                                                        Diese Felder müssen für jeden Artikel erkannt
                                                                        werden. Falsch erkannte Spalten können manuell
                                                                        korrigiert werden.
                                                                    </SmallText>
                                                                    <div
                                                                        style={{
                                                                    ...
                                                                    <div
                                                                        style={{
                                                                            marginTop: 8,
                                                                            fontSize: 12,
                                                                            color: requiredPresence.missing.length
                                                                                ? '#92400E'
                                                                                : '#166534',
                                                                        }}
                                                                    >
                                                                ...
                                                                >
                                                                    ...
                                                                    <SmallText>
                                                                        Diese Felder sind nicht zwingend, verbessern
                                                                        aber Qualität und Score. Falsch erkannte Spalten
                                        ...
                                        {(showAllChecks || stage2Status !== 'ok') && (
                                            <StepCard
                                                ...
                                                status={stage2Status}
                                                subtitle="Wir prüfen doppelte EAN, Produkttitel und Offer IDs"
                                            >
                                                {!headers.length ? (
                                                    ...
                                                    <>
                                                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                                            {[
                                                                {
                                                                    label: 'EAN',
                                                                    hasDups: duplicateEans.length > 0,
                                                                    found: !!eanColumn,
                                                                },
                                                                {
                                                                    label: 'Titel',
                                                                    hasDups: duplicateTitleRows.length > 0,
                                                                    found: !!titleColumn,
                                                                },
                                                                {
                                                                    label: 'Offer ID',
                                                                    hasDups: duplicateSellerOfferIds.length > 0,
                                                                    found: !!sellerColumn,
                                                                },
                                                            ].map((t) => {
                                                                        ...
                                                                        items={groupByValueWithEans(
                                                                            ...
                                                                            .filter(
                                                                                (g) =>
                                                                                    ...
                                                                                    g.eans.some((ean) =>
                                                                                        eanSearchTerms.some((t) =>
                                                                                            String(ean).includes(t),
                                                                                        ),
                                                                                    ),
                                                                            )
                                                                            .map(
                                                                                (g) =>
                                                                                    `${g.value} – ${g.eans.length} EANs: ${g.eans.join(', ')}`,
                                                                            )}
                                                                        tone="warn"
                                                                    />
                                                                )}
                                        ...
                                        {hasOptionalShippingFindings && (
                                            <StepCard
                                                title="Spalten und Inhalt prüfen"
                                                status={stage3Status}
                                            >
                                                {!headers.length ? (
                                                    <SmallText>
                                                        Bitte CSV hochladen, um optionale Felder und Versand zu prüfen.
                                                    </SmallText>
                                                ) : (
                                                    <>
                                                        {optionalFindings.missingEANs.length > 0 ? (
                                                            <div style={{ marginTop: 14 }}>
                                                                <CollapsibleList
                                                                    ...
                                                                    onItemClick={(label) => {
                                                                        const m =
                                                                            String(label).match(/Zeile (\d+)\s*$/);
                                                                        if (m)
                                                                            jumpToIssueTarget({
                                                                                rowIndex: parseInt(m[1], 10) - 1,
                                                                                column: eanColumn,
                                                                            });
                                                                    }}
                                                                />
                                                        ...
                                                        {(optionalFindings.missingEansByField.material.length > 0 ||
                                                            ...
                                                            <div style={{ marginTop: 12, display: 'grid', gap: 8 }}>
                                                                {optionalFindings.missingEansByField.material.length >
                                                                    0 && (
                                                                    <CollapsibleList
                                                                        title={`Material fehlt (${optionalFindings.missingEansByField.material.length})`}
                                                                        items={optionalFindings.missingEansByField.material
                                                                            .slice(0, 50)
                                                                            .map((ean) => String(ean))}
                                                                        tone="warn"
                                                                        onItemClick={(ean) =>
                                                                            jumpToEanWithColumn(ean, mapping.material)
                                                                        }
                                                                    />
                                                                )}
                                                                {optionalFindings.invalidMaterial?.length > 0 && (
                                                                    <CollapsibleList
                                                                        title="Material außerhalb erlaubter Werte"
                                                                        items={groupByValueWithEans(
                                                                            optionalFindings.invalidMaterial,
                                                                        ).slice(0, 50)}
                                                                        tone="warn"
                                                        ...
                                                        {(optionalFindings.missingEansByField.color.length > 0 ||
                                                            (optionalFindings.invalidColor?.length || 0) > 0) && (
                                                            <div style={{ marginTop: 12, display: 'grid', gap: 8 }}>
                                                                {optionalFindings.missingEansByField.color.length >
                                                                    0 && (
                                                                    <CollapsibleList
                                                                        title={`Farbe fehlt (${optionalFindings.missingEansByField.color.length})`}
                                                                        items={optionalFindings.missingEansByField.color
                                                                            .slice(0, 50)
                                                                            .map((ean) => String(ean))}
                                                                        tone="warn"
                                                                        onItemClick={(ean) =>
                                                                            jumpToEanWithColumn(ean, mapping.color)
                                                                        }
                                                                    />
                                                                )}
                                                                {optionalFindings.invalidColor?.length > 0 && (
                                                                    <CollapsibleList
                                                                        title="Farbe außerhalb erlaubter Werte"
                                                                        items={groupByValueWithEans(
                                                                            optionalFindings.invalidColor,
                                                                        ).slice(0, 50)}
                                                                        tone="warn"
                                                                        onItemClick={(ean) =>
                                                                            jumpToEanWithColumn(ean, mapping.color)
                                                        ...
                                                        {(optionalFindings.missingEansByField.delivery_includes.length >
                                                            ...
                                                            <div style={{ marginTop: 12, display: 'grid', gap: 8 }}>
                                                                {optionalFindings.missingEansByField.delivery_includes
                                                                    .length > 0 && (
                                                                    <CollapsibleList
                                                                        title={`Lieferumfang fehlt (${optionalFindings.missingEansByField.delivery_includes.length})`}
                                                                ...
                                                                {optionalFindings.invalidDeliveryIncludes?.length >
                                                                    0 && (
                                                                    <CollapsibleList
                                                                        title="Lieferumfang-Format ungültig"
                                                                        items={groupByValueWithEans(
                                                                            optionalFindings.invalidDeliveryIncludes,
                                                                        ).slice(0, 50)}
                                                                        tone="warn"
                                                                        onItemClick={(ean) =>
                                                                            jumpToEanWithColumn(
                                                        ...
                                                        (optionalFindings.missingEansByField.delivery_time?.length >
                                                            ...
                                                            <div style={{ marginTop: 12, display: 'grid', gap: 8 }}>
                                                                {optionalFindings.missingEansByField.delivery_time
                                                                    ?.length > 0 && (
                                                                    <CollapsibleList
                                                                        title={`Lieferzeit fehlt (${optionalFindings.missingEansByField.delivery_time.length})`}
                                                                ...
                                                                {optionalFindings.invalidDeliveryTime?.length > 0 && (
                                                                    <CollapsibleList
                                                                        title="Lieferzeit-Format ungültig"
                                                                        items={groupByValueWithEans(
                                                                            optionalFindings.invalidDeliveryTime,
                                                                        )
                                                                            .slice(0, 50)
                                                                            .map(
                                                                                (g) =>
                                                                                    `${g.value || '(leer)'} – ${g.eans.length} EANs: ${g.eans.join(', ')}`,
                                                                            )}
                                                                        tone="warn"
                                                                        onItemClick={(ean) =>
                                                                            jumpToEanWithColumn(
                                                        ...
                                                        {optionalFindings.templateValueHits?.length > 0 && (
                                                            <div style={{ marginTop: 12 }}>
                                                                <CollapsibleList
                                                                    ...
                                                                    items={groupByValueWithEans(
                                                                        optionalFindings.templateValueHits,
                                                                    )
                                                                        .slice(0, 50)
                                                                        .map(
                                                                            (g) =>
                                                                                `${g.value} (Spalte ${g.column || '?'}): ${g.eans.slice(0, 5).join(', ')}`,
                                                                        )}
                                                                    tone="warn"
                                                                />
                                                            </div>
                                                        ...
                                                        {mapping.washable_cover &&
                                                            optionalFindings.invalidWashableCover.length > 0 && (
                                                                <div style={{ marginTop: 12 }}>
                                                                    <CollapsibleList
                                                                        title="Waschbarer Bezug ungültig"
                                                                        items={groupByValueWithEans(
                                                                            optionalFindings.invalidWashableCover,
                                                                        )
                                                                            .slice(0, 50)
                                                                            .map(
                                                                                (g) =>
                                                                                    `${g.value} – ${g.eans.length} EANs`,
                                                                            )}
                                                                        tone="warn"
                                                                        onItemClick={(ean) =>
                                                                            jumpToEanWithColumn(
                                                        ...
                                                        {mapping.mounting_side &&
                                                            !/(montage|instruction|anleitung)/i.test(
                                                                String(mapping.mounting_side),
                                                            ) &&
                                                            optionalFindings.invalidMountingSide.length > 0 && (
                                                                <div style={{ marginTop: 12 }}>
                                                                    <CollapsibleList
                                                                        title="Montageseite ungültig"
                                                                        items={groupByValueWithEans(
                                                                            optionalFindings.invalidMountingSide,
                                                                        )
                                                                            .slice(0, 50)
                                                                            .map(
                                                                                (g) =>
                                                        ...
                                                        (optionalFindings.missingShipping.length > 0 ||
                                                            ...
                                                            <div style={{ marginTop: 12, display: 'grid', gap: 8 }}>
                                                                {optionalFindings.missingShipping.length > 0 && (
                                                                    <CollapsibleList
                                                                        title={`Versandart fehlt (${optionalFindings.missingShipping.length})`}
                                                                ...
                                                                {optionalFindings.invalidShipping.length > 0 && (
                                                                    <CollapsibleList
                                                                        title="Versandart ungültig"
                                                                        items={groupByValueWithEans(
                                                                            optionalFindings.invalidShipping,
                                                                        )
                                                                            .slice(0, 50)
                                                                            .map(
                                                                                (g) =>
                                                        ...
                                                        {optionalFindings.scientificEans.length > 0 ? (
                                                            ...
                                                            >
                                                                ...
                                                                >
                                                                    Einige EAN Werte sehen nach wissenschaftlicher
                                                                    Schreibweise aus.
                                                                </div>
                                                                <div style={{ marginTop: 10 }}>
                                                                    <CollapsibleList
                                                                        title="Betroffene EAN"
                                                                        items={optionalFindings.scientificEans.filter(
                                                                            (x) =>
                                                                                !eanSearchTerms.length ||
                                                                                eanSearchTerms.some((t) =>
                                                                                    String(x).includes(t),
                                        ...
                                        >
                                            {!headers.length ? (
                                                ...
                                                <>
                                                    ...
                                                    >
                                                        <Pill tone={imageColumns.length ? 'ok' : 'warn'}>
                                                            {imageColumns.length
                                                                ? `Bildspalten ${imageColumns.length}`
                                                                : 'Keine Bildspalten erkannt'}
                                                        </Pill>
                                                    </div>
                                                    ...
                                                    {(() => {
                                                        ...
                                                        if (!keys.length)
                                                            return (
                                                                <SmallText>
                                                                    Keine Bildinformationen ermittelt.
                                                                </SmallText>
                                                            );
                                                        return (
                                                            <div
                                                    ...
                                                    {(() => {
                                                        const filtered = imageSamples.filter(
                                                            (s) =>
                                                                !eanSearchTerms.length ||
                                                                eanSearchTerms.some((t) => String(s.id).includes(t)),
                                                        );
                                                        ...
                                                        if (!shown.length)
                                                            return (
                                                                <div style={{ marginTop: 12 }}>
                                                                    <SmallText>
                                                                        Keine Produkte mit Bildlinks gefunden.
                                                                    </SmallText>
                                                                </div>
                                                            );
                                                        return (
                                                            ...
                                                            >
                                                                ...
                                                                {brokenImageIds.length > 0 && (
                                                                    <div style={{ fontSize: 11, color: '#92400E' }}>
                                                                        {brokenImageIds.length} Produkte mit
                                                                        fehlerhaften Bild-Links:{' '}
                                                                        {brokenImageIds.slice(0, 5).join(', ')}
                                                                        {brokenImageIds.length > 5 ? ' ...' : ''}
                                                                    </div>
                                                                )}
                                                            </div>
                                                        );
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                                <input
                                                                    ...
                                                                    onChange={(e) =>
                                                                        setEanImageViewerInput(e.target.value)
                                                                    }
                                                                    placeholder="EAN eingeben"
                                                                    onKeyDown={(e) => {
                                                                        if (e.key === 'Enter')
                                                                            openEanImageViewer(eanImageViewerInput);
                                                                    }}
                                                                    style={{
                                                                        flex: 1,
                                                                        minWidth: 0,
                                                                ...
                                                                <button
                                                                    ...
                                                                    style={{
                                                                        ...
                                                                        fontSize: 12,
                                                                        fontWeight: 600,
                                                                        cursor: eanImageViewerInput.trim()
                                                                            ? 'pointer'
                                                                            : 'not-allowed',
                                                                    }}
                                                                >
                    ...
                    >
                        <div style={{ display: 'grid', gap: 10 }}>
                            ...
                            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                                ...
                                <SmallText>
                                    Die Daten werden nur gespeichert, wenn die Feature-Route genutzt wird.
                                </SmallText>
                            </div>
                            ...
                            {analyticsStats ? (
                                <div style={{ display: 'grid', gap: 12 }}>
                                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                                        ...
                                        <Pill tone={analyticsStats.totalClaudeUsed ? 'warn' : 'ok'}>
                                            Claude: {analyticsStats.totalClaudeUsed ?? 0}
                                        </Pill>
                                    ...
                                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                                        ...
                                        {typeof analyticsStats.offerCountAvg === 'number' ? (
                                            <Pill tone="info">
                                                Angebote im Schnitt: {analyticsStats.offerCountAvg.toFixed(1)}
                                            </Pill>
                                        ) : null}
                                    </div>
                                    ...
                                    >
                                        <div
                                            style={{ fontSize: 13, fontWeight: 900, color: '#111827', marginBottom: 8 }}
                                        >
                                            Letzte Tage
                                        </div>
                                        {Array.isArray(analyticsStats.last30Days) &&
                                        analyticsStats.last30Days.length ? (
                                            <div
    ...
    if (route === 'mapping') {
        ...
        function autoDetectCheck24Attr(col) {
            ...
            if (/size_seat_height|seat_height|sitzhoeche|sitzhöhe/.test(n))
                return 'Maße & Gewicht > Sitzhöhe (19) in mm';
            if (/size_lying_surface|lying_surface|liegeflaeche|liegeflache/.test(n))
                return 'Maße & Gewicht > Liegefläche (15) text';
            // Name / Title
            if (/^(name|title|product_name|produkt_name|produktname|bezeichnung|artikelname)$/.test(n))
                return 'Allgemein > Name (1) text';
            // Description
            if (/beschreibung|description|produktbeschreibung|produkt_beschreibung/.test(n))
                return 'Allgemein > Beschreibung (2) text';
            // Model
            if (/^(modell|model|model_number|modellnummer)$/.test(n)) return 'Allgemein > Modell (3) text';
            // Manufacturer number
            if (/herstellernummer|hersteller_nummer|manufacturer_number|mpn|sku/.test(n))
                return 'Allgemein > Herstellernummer (4) text';
            // Delivery includes
            if (/delivery_includes|lieferumfang|lieferinhalt|lieferung_inhalt|scope_of_delivery/.test(n))
                return 'Lieferung > Lieferumfang (11) text';
            // Color
            if (/^(farbe|color|colour|product_color|produktfarbe)$/.test(n)) return 'Farbe & Design > Farbe (12) text';
            // Brand / Marke
            if (/^(brand|marke|hersteller|manufacturer|manufacturer_name)$/.test(n))
                return 'Allgemein > Marke (50) text';
            // Material
            ...
            // Dimensions combined
            if (/abmessungen|dimensions|abmessung|masse$|maße$|groesse$|groesse_produkt/.test(n))
                return 'Maße & Gewicht > Abmessungen (14) text';
            // Height
            if (/^(hoehe|height|height_mm|product_height|h_mm|hoehe_mm)$/.test(n) || /^h$/.test(n))
                return 'Maße & Gewicht > Höhe (5) in mm';
            // Depth
            if (/^(tiefe|depth|depth_mm|product_depth|t_mm|tiefe_mm)$/.test(n) || /^t$/.test(n))
                return 'Maße & Gewicht > Tiefe (6) in mm';
            // Width
            if (/^(breite|width|width_mm|product_width|b_mm|breite_mm)$/.test(n) || /^b$/.test(n))
                return 'Maße & Gewicht > Breite (7) in mm';
            // Diameter
        ...
        function getNormalizerTip(fieldLabel) {
            ...
            if (/beschreibung|description/.test(n)) {
                return {
                    text: 'HTML vorhanden? → "HTML in Markdown umwandeln" (formatiert besser) · Nur Text? → "HTML entfernen"',
                    bg: '#FFFBEB',
                    border: '#FCD34D',
                    color: '#92400E',
                };
            ...
            if (
                /size_|abmessungen|dimensions|masse|maße|hoehe|height|tiefe|depth|breite|width|durchmesser|diameter|groesse|gewicht|weight|volumen|volume|liegeflaeche|liegeflache/.test(
                    n,
                )
            ) {
                return {
                    text: '"Interpretiere als numerisch" (konvertiert in mm)',
                    bg: '#FFFBEB',
                    border: '#FCD34D',
                    color: '#92400E',
                };
            ...
            if (/versand|delivery|lieferzeit|liefermode|shipping/.test(n)) {
                return {
                    text: '"Versandart ermitteln" (für delivery_mode)',
                    bg: '#FFFBEB',
                    border: '#FCD34D',
                    color: '#92400E',
                };
        ...
        function autoDetectImageColumn(imgNum) {
            const patterns = [
                imgNum === 1
                    ? /^image_url$|^img_url$|^bild$|^bild_1$|^image$/
                    : new RegExp(
                          `^image_url\\s+${imgNum}$|^image_url_${imgNum}$|^img_url\\s+${imgNum}$|^img_url_${imgNum}$|^bild_${imgNum}$|^bild\\s+${imgNum}$`,
                      ),
            ];
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '32px 20px' }}>
                    ...
                    <div style={{ marginBottom: 40, paddingBottom: 24, borderBottom: '2px solid #E5E7EB' }}>
                        ...
                        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                            <button
                                ...
                                onFocus={(e) => (e.target.style.background = '#1a37a0')}
                                onMouseOut={(e) => (e.target.style.background = '#1E40AF')}
                                onBlur={(e) => (e.target.style.background = '#1E40AF')}
                            >
                                📁 Datei auswählen
                            </button>
                            <span
                                style={{
                                    fontSize: 13,
                                    color: mappingFileName ? '#111827' : '#9CA3AF',
                                    fontWeight: mappingFileName ? 600 : 400,
                                }}
                            >
                        ...
                        {mappingError && (
                            <div style={{ color: '#DC2626', fontSize: 12, marginTop: 8 }}>{mappingError}</div>
                        )}
                    </div>
                    ...
                    <div style={{ marginBottom: 40 }}>
                        ...
                        <div style={{ fontSize: 14, color: '#6B7280', lineHeight: 1.6, maxWidth: 760 }}>
                            Dieses Tool hilft Ihnen dabei, Ihren Produktfeed mit dem CHECK24 Content-System zu
                            verbinden. Das Mapping legt fest, welche Spalten aus Ihrem Feed welchen Feldern bei CHECK24
                            entsprechen — von der Produktidentifikation bis zu allen sichtbaren Attributen auf der
                            Produktdetailseite.
                        </div>
                        <div style={{ display: 'flex', gap: 24, marginTop: 20 }}>
                            {[
                                {
                                    step: '1',
                                    title: 'Feed hochladen',
                                    desc: 'CSV- oder Excel-Datei (.xlsx) mit Ihren Produktdaten',
                                    icon: '📁',
                                },
                                {
                                    step: '2',
                                    title: 'Produktidentifikation',
                                    desc: 'Technische Pflichtfelder mappen',
                                    icon: '🔑',
                                },
                                {
                                    step: '3',
                                    title: 'Attributmapping',
                                    desc: 'Sichtbare Produktattribute zuordnen',
                                    icon: '🏷️',
                                },
                                {
                                    step: '4',
                                    title: 'Bilder & Dokumente',
                                    desc: 'Bildquellen und Dateien mappen',
                                    icon: '🖼️',
                                },
                            ].map((s) => (
                                <div
                    ...
                    {mappingHeaders.length === 0 && (
                        ...
                        >
                            ...
                            <div style={{ fontSize: 12, color: '#1E40AF', marginTop: 4 }}>
                                Laden Sie Ihre CSV- oder Excel-Datei (.xlsx) oben hoch. Das System erkennt die Spalten
                                automatisch und schlägt passende Zuordnungen vor. Je mehr Attribute korrekt gemappt
                                sind, desto besser werden Ihre Produkte bei CHECK24 angezeigt.
                            </div>
                        </div>
                    )}
                        ...
                        <>
                            ...
                            <div style={{ marginBottom: 48 }}>
                                ...
                                <div style={{ marginBottom: 40 }}>
                                    ...
                                    >
                                        <div
                                            style={{ fontSize: 13, fontWeight: 700, color: '#1E40AF', marginBottom: 6 }}
                                        >
                                            🔑 Schritt 2 — Produktidentifikation
                                        </div>
                                        <div style={{ fontSize: 12, color: '#374151', lineHeight: 1.6 }}>
                                            Diese Felder sind technische Pflichtfelder für den Import. Sie werden{' '}
                                            <strong>nicht direkt auf der Produktseite angezeigt</strong>, aber sind
                                            notwendig damit CHECK24 das Produkt korrekt verarbeiten und eindeutig
                                            identifizieren kann.
                                        </div>
                                    ...
                                    >
                                        <div>
                                            ...
                                            <div style={{ fontSize: 11, color: '#6B7280', marginTop: 4 }}>
                                                ...
                                                ></span>
                                                Blau = manuell gesetzt
                                            </div>
                                        </div>
                                        <button
                                            type="button"
                                            onClick={() => setProduktIdentifikationMappings({})}
                                            style={{
                                                fontSize: 11,
                                                padding: '4px 12px',
                                                border: '1px solid #D1D5DB',
                                        ...
                                        return (
                                            <div
                                                ...
                                                style={{
                                                    ...
                                                    background: isUserSet
                                                        ? '#F0F4FF'
                                                        : isAutoDetected && !isUserExplicitlyClearedIt
                                                          ? '#F0FDF4'
                                                          : idx % 2 === 0
                                                            ? '#F9FAFB'
                                                            : '#FFFFFF',
                                                    alignItems: 'center',
                                                    borderBottom: isUserSet
                                                        ? '1px solid #BFDBFE'
                                                        : isAutoDetected && !isUserExplicitlyClearedIt
                                                          ? '1px solid #BBF7D0'
                                                          : '1px solid #E5E7EB',
                                                    borderLeft: isUserSet
                                                        ? '3px solid #1E40AF'
                                                        : isAutoDetected && !isUserExplicitlyClearedIt
                                                          ? '3px solid #16A34A'
                                            ...
                                            >
                                                ...
                                                <select
                                                    value={selectedValue || ''}
                                                    onChange={(e) =>
                                                        setProduktIdentifikationMappings((prev) => ({
                                                            ...prev,
                                                ...
                                                >
                                                    <option value="">
                                                        {isUserExplicitlyClearedIt
                                                            ? '-- Wählen --'
                                                            : autoDetectedValue || '-- Wählen --'}
                                                    </option>
                            ...
                            <div style={{ marginBottom: 48 }}>
                                ...
                                >
                                    <div style={{ fontSize: 13, fontWeight: 700, color: '#1E40AF', marginBottom: 8 }}>
                                        🏷️ Schritt 3 — Attributmapping: Was Kunden auf CHECK24 sehen
                                    </div>
                                    <div style={{ fontSize: 12, color: '#374151', lineHeight: 1.7, marginBottom: 16 }}>
                                        Das Attributmapping bestimmt, welche Produkteigenschaften aus Ihrem Feed als
                                        strukturierte Attribute auf der <strong>Produktdetailseite bei CHECK24</strong>{' '}
                                        angezeigt werden. Kunden nutzen diese Informationen aktiv, um Produkte zu
                                        vergleichen und Kaufentscheidungen zu treffen — je vollständiger das Mapping,
                                        desto besser die Konversion.
                                    </div>
                                        ...
                                        <div>
                                            ...
                                            <div style={{ display: 'grid', gap: 8 }}>
                                                {[
                                                    {
                                                        icon: '1️⃣',
                                                        text: 'Jede Zeile = eine Spalte aus Ihrem Feed (z.B. "material", "color", "height_mm")',
                                                    },
                                                    {
                                                        icon: '2️⃣',
                                                        text: 'Wählen Sie das passende CHECK24 Attribut rechts (z.B. "Material > Material (23)")',
                                                    },
                                                    {
                                                        icon: '3️⃣',
                                                        text: 'Das System überträgt die Werte aus Ihrem Feed in die entsprechenden Felder bei CHECK24',
                                                    },
                                                    {
                                                        icon: '4️⃣',
                                                        text: 'Nicht gemappte Felder erscheinen nicht auf der Produktseite — also so viele wie möglich mappen!',
                                                    },
                                                ].map(({ icon, text }) => (
                                                    <div
                                                        key={icon}
                                            ...
                                            >
                                                <strong>💡 Tipp:</strong> Mappen Sie besonders: Maße, Material, Farbe,
                                                Lieferumfang und Marke — das sind die am häufigsten genutzten
                                                Filterattribute bei CHECK24.
                                            </div>
                                        </div>
                                        ...
                                        <div style={{ flexShrink: 0 }}>
                                            ...
                                            >
                                                {[
                                                    {
                                                        label: 'Maße (HxTxB)',
                                                        value: '81,5 x 19 x 152 cm',
                                                        attr: 'Maße & Gewicht > Abmessungen',
                                                    },
                                                    {
                                                        label: 'Material',
                                                        value: 'MDF mit wasserbasierter Lackierung',
                                                        attr: 'Material > Material',
                                                    },
                                                    {
                                                        label: 'Lieferumfang',
                                                        value: '1 x Heizkörperverkleidung',
                                                        attr: 'Lieferung > Lieferumfang',
                                                    },
                                            ...
                                            <div style={{ marginTop: 8, display: 'grid', rowGap: 4 }}>
                                                {[
                                                    {
                                                        label: 'Maße (HxTxB)',
                                                        attr: 'Maße & Gewicht > Abmessungen (14)',
                                                    },
                                ...
                                <div style={{ marginBottom: 16, paddingBottom: 12, borderBottom: '1px solid #E5E7EB' }}>
                                    ...
                                    <div style={{ fontSize: 11, color: '#6B7280', marginTop: 4 }}>
                                        Ordnen Sie jede Feed-Spalte dem entsprechenden CHECK24-Attribut zu. Das System
                                        erkennt passende Attribute automatisch — überprüfen und ergänzen Sie die
                                        Zuordnungen manuell.
                                    </div>
                                </div>
                                {mappingHeaders.length === 0 && (
                                ...
                                <div
                                    style={{
                                        ...
                                        paddingRight: 16,
                                        borderBottom: '1px solid #E5E7EB',
                                        background: mappingHeaders.length === 0 ? '#F9FAFB' : '#FFFFFF',
                                    }}
                                >
                                    <div style={{ fontSize: 12, fontWeight: 700, color: '#111827' }}>
                                        Quellspalte des Feeds
                                    </div>
                                    <div style={{ fontSize: 12, fontWeight: 400, color: '#9CA3AF' }}>Preview</div>
                                    <div style={{ fontSize: 12, fontWeight: 700, color: '#111827' }}>
                                        Mapping auf CHECK24 Attribut
                                    </div>
                                ...
                                {attributeMappingFields.length > 0 &&
                                    attributeMappingFields.map((field, idx) => {
                                        ...
                                        const displayVal = isUserSet
                                            ? userVal
                                            : !isCleared && field.autoAttr
                                              ? field.autoAttr
                                              : '';
                                            ...
                                            <div key={field.label}>
                                                ...
                                                >
                                                    ...
                                                    <div style={{ fontSize: 12, color: '#9CA3AF' }}>
                                                        {field.feedValue
                                                            ? String(field.feedValue).substring(0, 35) +
                                                              (String(field.feedValue).length > 35 ? '...' : '')
                                                            : '-'}
                                                    </div>
                                                    <input
                                                        type="text"
                                                    ...
                                                    <span
                                                        ...
                                                        style={{
                                                            ...
                                                            color: displayVal ? '#DC2626' : '#D1D5DB',
                                                            fontSize: 14,
                                                            fontWeight: 700,
                                                        }}
                                                ...
                                                {tip && (
                                                    <div
                                                        style={{
                                                            ...
                                                            borderRight: `1px solid ${tip.border}`,
                                                            borderBottom: `1px solid ${tip.border}`,
                                                            padding: '8px 12px',
                                                            fontSize: 11,
                                                            color: tip.color,
                                                            marginBottom: 0,
                                        ...
                                        (num) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <span style={{ cursor: 'pointer', color: '#9CA3AF', fontSize: 14 }}>
                                                        ⓘ
                                                    </span>
                                                </div>
                                            );
                            ...
                            <div style={{ marginBottom: 32 }}>
                                ...
                                >
                                    ...
                                    <div style={{ fontSize: 12, fontWeight: 700, color: '#111827' }}>
                                        Normalizer <span style={{ color: '#1E40AF' }}>Glossar</span>
                                    </div>
                                </div>
                                        ...
                                        >
                                            ...
                                            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                                <input
                                                    ...
                                                    value={displayVal}
                                                    onChange={(e) =>
                                                        setImageMappings((prev) => ({ ...prev, [num]: e.target.value }))
                                                    }
                                                    style={{
                                                        flex: 1,
                                                        padding: '8px 10px',
                                                ...
                                                {displayVal && (
                                                    <span
                                                        onClick={() =>
                                                            setImageMappings((prev) => ({ ...prev, [num]: '' }))
                                                        }
                                                        style={{
                                                            cursor: 'pointer',
                                                            color: '#DC2626',
                            ...
                            <div style={{ marginBottom: 32 }}>
                                ...
                                {['Aufbauanleitung', 'Energieeffizienzlabel', 'Produktdatenblatt'].map(
                                    (docType, idx) => (
                                        <div
                                            key={docType}
                                            style={{
                            ...
                            <div style={{ marginBottom: 32 }}>
                                <div style={{ fontSize: 16, fontWeight: 600, color: '#1E40AF', marginBottom: 24 }}>
                                    Feed Import Filter
                                </div>
